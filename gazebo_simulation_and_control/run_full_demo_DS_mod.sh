...
SCRIPT_DIR="$CATKIN_WS/src/$PROJECT/gazebo_simulation_and_control/movewithhead/script"
SESSION="ds_demo"
...
tmux send-keys -t $SESSION:1 \
  "cd $CATKIN_WS && source devel/setup.bash && chmod +x $SCRIPT_DIR/dualPosition.py && rosrun movewithhead script/dualPosition.py" C-m
...
tmux send-keys -t $SESSION:4 \
  "cd $CATKIN_WS && source devel/setup.bash && chmod +x $SCRIPT_DIR/moveit_ds_modulation_executor.py && rosrun movewithhead script/moveit_ds_modulation_executor.py" C-m
...
tmux send-keys -t $SESSION:5 \
  "cd $CATKIN_WS && source devel/setup.bash && chmod +x $SCRIPT_DIR/run_a_demo.py && python3 $SCRIPT_DIR/run_a_demo.py 1" C-m
